...
from Node_level_Models.helpers.func_utils import  get_split, get_total_size, agg_local_proto_func, agg_global_proto_func, visualize_node_embeddings, visualize_node_embeddings_new_data, ContrastiveDiversityLoss
...
def construct_graph_with_adj_old(node_logits, adj_logits, real_labels):
    """
    ...
    """
    # Generate edges between nodes of the same class (including self-loops)
    edge_index = []
    edge_attr = []
    ...
    for label in unique_labels:
        ...
        src = indices.repeat_interleave(len(indices))
        dst = indices.repeat(len(indices))
        edge_index.append(torch.stack([src, dst], dim=0))
        weights = torch.where(
            src == dst,
            torch.ones_like(src, dtype=adj_logits.dtype),
            adj_logits[src, dst]
        )
...
def construct_graph_with_adj(node_logits, adj_logits, real_labels, edge_thre=0.6):
    """
    Construct the graph data structure where nodes of the same class are connected by edges (including self-loops),
    and extract edge weights from adj_logits
    ...
    k = max(1, int(len(non_diag_values) * 0.75))
...
def compute_alpha(grad1, grad2):
    ...
    try:
        numerator = g2 @ g2 - g1 @ g2 
        denominator = (g1 @ g1 + g2 @ g2) - 2 * g1 @ g2
...
def project_gradient(grad_sem, grad_div):
    ...
    norm_sem_sq = torch.sum(grad_sem **2) + 1e-12
    
    if dot_product < 0:
        # Compute projection coefficients
        proj_coeff = dot_product / norm_sem_sq
        # Compute the projected gradient (remove components opposite to the semantic gradient)
        projected_grad = grad_div - proj_coeff * grad_sem
        return projected_grad
    else:
        # Use the original gradient directly if directions are consistent
        return grad_div
...
def main(args, logger):
    ...
    if (args.dataset == 'Cora' or args.dataset == 'Pubmed'):
        ...
        dataset = Reddit(root='/home/zheyu/data/Reddit/', \
                          transform=T.LargestConnectedComponents())
    ...
    generator = model_construct(args, "MultiFedKD_Generator", client_data[0], device, nclass).to(device)
    generator_optimizer = Adam(generator.parameters(), lr=0.01, weight_decay=args.weight_decay)

    for g in range(args.k_generators):
        if g == args.k_generators -1:
            print(f"Generator {g} is responsible for classes: {g*generator.classes_per_generator}-{nclass-1}")
        else:
            print(f"Generator {g} is responsible for classes: {g*generator.classes_per_generator}-{(g+1)*generator.classes_per_generator - 1}")
    ...
    if args.class_num <= 15:
        ...
        for class_i in range(args.class_num):
            c_cnt[class_i] = int(args.sample_num * 1 / args.class_num)
        c_cnt[-1] += args.sample_num - sum(c_cnt)
        ...
        for class_i in range(args.class_num):
            for _ in range(c_cnt[class_i]):
                ...
                ptr += 1
        print("Generated pseudo nodes labels: ", label_distribution)
    ...
    for epoch in range(args.epochs):
        ...
        # 1.0 For the Reddit dataset, generate a new label distribution each time
        if args.class_num > 15:
            selected_classes_num = 15
            selected_classes = torch.randperm(args.class_num)[:selected_classes_num]
            samples_per_class = args.sample_num // 15
            remainder = args.sample_num % 15
            label_distribution = torch.zeros(args.sample_num).to(device).long()
            ptr = 0
            for i, class_i in enumerate(selected_classes):
                num_samples = samples_per_class + (1 if i < remainder else 0)
                label_distribution[ptr:ptr+num_samples] = class_i
                ptr += num_samples

        # 1.1 Generate the public dataset
        ...
        pseudo_graph = construct_graph_with_adj(node_logits=node_logits.detach(), adj_logits=adj_matrix.detach(), real_labels=label_distribution)

        # 1.2 Put the newly generated data into the cache
        ...
        # 1.2 Model Aggregation: Weighted Multi-Teacher Distillation
        local_class_scores = []
        teacher_weights = []
        ...
        # 1.3 Model Distribution: Student Model Training, Student Model Starts Learning via Distillation
        for j in range(args.num_workers):
        ...
        for j in range(args.num_workers):
            #-------------------------------- Below is the Model Training Section --------------------------------
            ...
            if epoch > warmup and all_class_flag:
                ...
                # 2.0.1 Target nodes are pseudo nodes
                ...
                beta = torch.distributions.beta.Beta(1, 2)
        ...
        for _ in range(10):
            ...
            z = torch.randn((args.sample_num, args.noise_dim)).to(device)
            node_logits, adj_matrix, z_c = generator.forward(z=z, c=label_distribution)
            pseudo_graph = construct_graph_with_adj(node_logits=node_logits, adj_logits=adj_matrix, real_labels=label_distribution)
            for client_id in range(args.num_workers):
                ...
                if args.use_GAN:
                    discriminator_model_list[client_id].eval()
                    discriminator_pred = discriminator_model_list[client_id].forward(pseudo_graph.x.to(device), pseudo_graph.edge_index.to(device), pseudo_graph.edge_weight.to(device))
                    if client_id == 0 and _ == 0:
                        print("D(G(z)) mean:", discriminator_pred.mean().item())
                        logger.log({"Round": epoch, "discriminator_pred": discriminator_pred})
                    real_labels = torch.ones(discriminator_pred.size(0), 1, device=device)
                    loss_real += 1/(args.num_workers) * nn.BCELoss()(discriminator_pred, real_labels)
            ...
            print("Generator's loss: loss_sem:{} loss_div:{} loss_real:{}".format(loss_sem, loss_div, loss_real))
    ...
    return acc_global, round_reach_target_acc, max_accuracy
